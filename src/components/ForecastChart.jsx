...
const CustomTooltip = ({ active, payload, label, t, i18n }) => {
    ...
    return (
      <div className="bg-paper border border-rule p-3 rounded shadow-lg font-mono text-sm min-w-[150px]">
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-ink-soft mr-4">{t('forecastChart.count')}</span>
        ...
        {isForecast && (
          <div className="flex items-center justify-between mb-1 text-xs text-ink-soft">
            <span className="mr-4">{t('forecastChart.range')}</span>
            <span>{numFormatter.format(Math.round(data.range[0]))} – {numFormatter.format(Math.round(data.range[1]))}</span>
          </div>
        )}
        <div className="flex items-center justify-between">
          <span className="text-ink-soft mr-4">{t('forecastChart.status')}</span>
          <span className={`uppercase text-[10px] tracking-wider font-bold ${isForecast ? 'text-signal' : 'text-ink'}`}>
            {isForecast ? t('forecastChart.predicted') : t('forecastChart.recorded')}
          </span>
        </div>
        {isForecast && (
          <div className="mt-2 pt-2 border-t border-rule text-[10px] text-ink-soft leading-tight">
            {t('forecastChart.historicalPattern')}
          </div>
        )}
      </div>
    );
...
export default function ForecastChart({ history = [], forecast = [] }) {
  ...
  const chartData = useMemo(() => {
    ...
    const dateFormatter = new Intl.DateTimeFormat(i18n.resolvedLanguage || 'en', { month: 'short', day: 'numeric', numberingSystem: 'latn' });
    ...
    return data;
  }, [history, forecast, i18n.resolvedLanguage]);
